...
import { Panel } from './classes';
import type { DirectionButton } from './types/MiscTypes';
...
export const signalDirectionButtons: Writable<DirectionButton[]> = writable([
  {
    transform: 'rotate(270) scale(.8)',
    next: {
      panelDirectionArray: [
        { x: 0, y: -1, snapPointIndexes: { origin: 0, destination: 1 } },
        { x: 1, y: 0, snapPointIndexes: { origin: 0, destination: 0 } },
        { x: 0, y: 1, snapPointIndexes: { origin: 1, destination: 0 } },
        { x: 1, y: 0, snapPointIndexes: { origin: 1, destination: 1 } }
      ]
    },
    origin: {
      x: -1,
